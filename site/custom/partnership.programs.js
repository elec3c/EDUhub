...
var btnProfileConclusionAgreementSave  = $('#btnProfileConclusionAgreementSave');
...
btnProfileConclusionAgreementSave.on('click', function() {
    
    if(!isCheck('conclusion_agreement')){
            
        AjaxForm.Message.error('Выберите приемлемые способы заключения партнерских соглашений с Учебными центрами');    
        return false;
...
$(document).on('click','.generate-promocode-employee',function(e){
    
    e.preventDefault();
    
    var action = 'generatePromocodeEmployee';
    var user_id = $(this).data("user");
    var currGeneratePromocodeEmployeeBtn  = $(this);
    ...
    $.ajax({
		...
		success : function(req){
			...
			if(req.answer == 1) {
			   ...
			   partnershipSubmitMSG(true, req.msg);
			   $('#employee-promocode-' + user_id).val(req.link);
			   currGeneratePromocodeEmployeeBtn.removeClass();
               currGeneratePromocodeEmployeeBtn.addClass('btn w-all btn--red reset-promocode-employee');
               currGeneratePromocodeEmployeeBtn.attr('value', 'Сбросить промокод');
...
$(document).on('click','.reset-promocode-employee',function(e){
    
    e.preventDefault();
    
    var action = 'resetPromocodeEmployee';
    var user_id = $(this).data("user");
    var currResetPromocodeEmployeeBtn  = $(this);
    ...
    $.ajax({
		...
		success : function(req){
			...
			if(req.answer == 1) {
			   ...
			   currResetPromocodeEmployeeBtn.removeClass();
               currResetPromocodeEmployeeBtn.addClass('btn w-all generate-promocode-employee');
               currResetPromocodeEmployeeBtn.attr('value', 'Сгенерировать промокод');
