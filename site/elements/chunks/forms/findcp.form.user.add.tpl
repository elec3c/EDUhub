{set $ts = time()}
{set $email = $_modx->user.id | user:'email'}
<form action="{$_modx->resource.uri}" method="post" class="findcp__added-form" id="findcpFormUserAdd">
<input type="hidden" name="csrf-token" value="{$.session['csrf-token']}">
    ...
    <input type="hidden" name="form" value="Поиск компании авторизированным пользователем">
...
<input type="hidden" name="user_id" value="{$_modx->user.id}">
            ...
            <input type="hidden" class="input" name="email" placeholder="Укажите ваш email" value="{$email}">
        </div>
