...
<div class="cgcourse rollup-box">
    <div class="cghead leads__header lk__wraplr section__lr">
        <div class="title">
            ...
            <a href="{1174 | url}?partnership_id={$id}">{if $date_create}{$date_create|date_format:"%d.%m.%Y"}&nbsp;{$to_user_id|user:'fullname'}{/if} ({if $scope}{$scope}, {/if}{$count_employees}&nbsp;{$count_employees | declension : 'сотрудник|сотрудника|сотрудников'})&nbsp;[<b style={"color:"~$color}>{$prefix}</b>]</a>
            {else}
            <a href="{1174 | url}?partnership_id={$id}">{if $date_start}{$date_start|date_format:"%d.%m.%Y"}&nbsp;{$to_user_id|user:'fullname'}{/if} ({if $scope}{$scope}, {/if}{$count_employees}&nbsp;{$count_employees | declension : 'сотрудник|сотрудника|сотрудников'})&nbsp;[<b style={"color:"~$color}>{$prefix}</b>]</a>
            {/if}
        </div>
